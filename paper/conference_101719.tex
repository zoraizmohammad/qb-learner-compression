...
In the present implementation we take $\rho_0 = \lvert 0\rangle\!\langle 0\rvert^{\otimes
n}$ (a definite prior) and realize the evidence channel as a \emph{unitary} channel
$\mathcal{E}_x(\rho)=U(x;\theta,m)\,\rho\,U(x;\theta,m)^{\dagger}$ built from a data
re-uploading encoding of $x$ interleaved with trainable layers (Section~II-B). The belief
state therefore remains pure and the dynamics are exactly simulable and analytically
differentiable, which is what lets us train the learner with exact gradients and map the
capacity boundary precisely; extending $\mathcal{E}_x$ to non-unital channels that drive
the belief mixed is an immediate generalization within the same formalism
(\ref{eq:channel}).
