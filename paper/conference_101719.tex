...
stimulus-dependent evidence channel that updates the belief on a Pothos--Chater style
categorization task, with the posterior category read out by measurement. To tie this
...
can be removed before the learner stops being able to learn. Across three task difficulty
...
every matched budget ($4$--$12$ points), so \emph{which} interactions are kept matters,
not only how many. Finally, on a physical IBM device (\texttt{ibm\_fez}) the full circuit
...
compressing this learner we obtain an empirical trade-off between \emph{cognitive
capacity}---the learner's ability to update beliefs and categorize stimuli---and
\emph{circuit capacity}---the entangling structure of the underlying circuit under NISQ
constraints. The guiding question is concrete: \emph{how much of the circuit can we remove
before the quantum Bayesian learner stops being able to learn?}
...
measuring the updated belief state (Section~\ref{sec:readout}).
...
term (each active Heisenberg term costs two CNOTs, so the full circuit gives $N_{2q}=24$).
...
exact gradients from automatic differentiation through the belief-state simulation.
...
\cite{b16,b17}; the shallow, small-register design keeps optimization well-conditioned
against barren plateaus \cite{b18}.

\subsection{Controllable Categorization Task}
\label{sec:data}

To expose rather than saturate the capacity boundary we use a similarity-structured
categorization task in the spirit of the simplicity principle in human categorization
\cite{b19}: stimuli are points in $[0,1]^2$ and the category is the parity of a
checkerboard cell, $y=(\lfloor f x_0\rfloor + \lfloor f x_1\rfloor)\bmod 2$. Nearby
...
real transpiled count, averaged over the five seeds. Code and logs are in the public
...
(\ref{eq:loss}) as $\lambda$ grows. For small penalties the learner keeps a moderately
...
budget, by $4$--$12$ points, largest in the low-to-mid regime where structure is scarce
and must be allocated well. In cognitive terms, capacity reductions that respect the task
...
$156$-qubit device), mapping the accuracy--complexity frontier on-device across five
two-qubit budgets at $2048$--$4096$ shots. The outcome (Fig.~\ref{fig:hardware}) is
decisive and \emph{reverses} the simulated picture. In exact simulation the frontier is
...
frontier (Table~\ref{tab:density}) is again high and flat across two-qubit budgets and
collapses to near chance only when all entanglers are removed ($N_{2q}=0$): the capacity
boundary is intact. The hardest task again shows accuracy peaking at intermediate cost
($0.88$ at $N_{2q}=6$ vs $0.87$ at the full $N_{2q}=24$). And learned masks again beat
random masks at every matched budget (mean advantage $+0.043$, up to $+0.11$ at
$N_{2q}=6$). The conclusions---a sharp capacity boundary, free or beneficial structured
compression, and the primacy of structure over count---therefore hold for the genuine
quantum Bayesian belief-update mechanism, not only its pure-state surrogate.
...
Reading the entangling structure as a proxy for cognitive capacity, the experiments locate
a clear capacity boundary. A fully entangled belief register has more representational
...
The study is deliberately controlled: a two-qubit belief register, an exactly-simulated
unitary evidence channel, and a controllable synthetic task make every claim measurable and
reproducible, and the whole pipeline already runs end-to-end on physical IBM hardware
...
ZX-calculus methods \cite{b22,b23} and the Qiskit transpiler \cite{b24}; unlike those, our
penalty is folded into the learning objective and coupled to a per-term mask, which is what
lets the learned-versus-random ablation isolate the role of structure. Warm-starting and
related strategies \cite{b28} are complementary directions for the optimization itself. To
our knowledge, linking a quantum Bayesian learner's cognitive capacity to its post-transpile
circuit capacity---and validating the trade-off on real hardware---has not been examined
before.
...
learner can be compressed before it stops being able to learn. Building a learner that
...
to fit---and even prefer---the limits of present-day quantum hardware.
